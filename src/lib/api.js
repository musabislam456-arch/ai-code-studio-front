...
export function getAccessToken() { return localStorage.getItem("acs_token") || ""; }
export function setAccessToken(token) { localStorage.setItem("acs_token", token); }
...
export async function agentChatStream(payload, onEvent) {
  ...
  if (token) headers.Authorization = `Bearer ${token}`;
  const res = await fetch(BASE + "/agent/chat", { method: "POST", headers, body: JSON.stringify(payload) });
...
export const api = {
  ...
  githubStatus: (ws) => req("GET", `/workspace/${ws}/github`),
  createGithubRepo: (ws, payload) => req("POST", `/workspace/${ws}/github/create-repo`, payload),
  unlinkGithubRepo: (ws) => req("POST", `/workspace/${ws}/github/unlink`),
